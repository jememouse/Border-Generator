...
from fastapi.templating import Jinja2Templates # 可选：如果需要更复杂的HTML模板渲染
...
# 配置日志 (如果 image_generator.py 中未配置，可以在这里配置)
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# 创建 FastAPI 应用实例
app = FastAPI(title="图像生成器 API", description="根据参数生成带边框的图像")

# 挂载静态文件目录 (用于提供 HTML, CSS, JS 文件)
# 请求路径 /static/* 将会访问项目根目录下的 static/ 文件夹中的文件
app.mount("/static", StaticFiles(directory="static"), name="static")

# 可选：设置 Jinja2 模板引擎 (如果你的 HTML 结构复杂或需要动态数据)
# templates = Jinja2Templates(directory="static") # 假设 index.html 在 static 目录下
...
async def generate_image_style1(
    ...
    # 添加一些基本的验证规则 (例如, ge=大于等于, le=小于等于, gt=大于)
...
# 如果你想直接运行这个文件进行测试 (虽然通常我们用 uvicorn 命令)
